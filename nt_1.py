...
def load_data():
    ...
    # Drop unused columns
    drop_cols = [
        "Corporate Name","Tehsil","Pincode","PAN NO.",
...
with tabs[1]:
    ...
    st.plotly_chart(
        px.bar(top_dealers, x="Dealer", y="Leads", title="Top 10 Dealers by Leads"),
...
def top5(df, by):
    agg = (
        df.groupby(by)["Enquiry Stage"]
          .agg(total="count",
               won=lambda x: x.isin(won_stages).sum(),
               lost=lambda x: x.isin(lost_stages).sum())
          .reset_index()
    )
    agg["Conv %"] = (agg.won/agg.total*100).round(1)
    return agg.nlargest(5, "won")
            ...
            if st.button("Submit Lead", key=f"sub_{idx}"):
                ...
                if exists:
                    ...
                    for i, ans in enumerate((q1,q2,q3,q4,q5), start=1):
                        log_audit(
                            current_user, "Create", lead["Enquiry No"],
        ...
        if not opts:
            ...
            with st.form("update_form"):
                ...
                new_act = st.text_input(
                    "Next Action", value=row.get("Next Action",""),
                    key="update_action"
                )
...
if role=="Admin":
    with tabs[-1]:
        ...
        # (Add/Edit/Delete logic can go here)
